...
install -Dpm644 -t %{buildroot}/opt/atlassian/fisheye atlassian-fisheye.patch
...
FISHEYE_DOWNLOAD_URL=http://product-downloads.atlassian.com/software/fisheye/downloads/fisheye-4.8.11.zip
FISHEYE_DOWNLOAD_DEST=/tmp/atlassian-fisheye-4.8.11.tar.gz
FISHEYE_DOWNLOAD_CHECKSUM=4c7c248f6c6ccb9171c70570f6f981bf14a32a1748094acbed2efff8ef8342f3
...
find $FISHEYE_CATALINA -mindepth 1 | grep -v atlassian-fisheye.patch | xargs rm -rf || echo $?
TMP_DIR="$(mktemp -d)" && \
    unzip -d $TMP_DIR $FISHEYE_DOWNLOAD_DEST && \
    cp -rfT $TMP_DIR/* $FISHEYE_CATALINA && \
    rm -rf $TMP_DIR

cat $FISHEYE_CATALINA/atlassian-fisheye.patch | patch -p1
...
/opt/atlassian/fisheye/atlassian-fisheye.patch
